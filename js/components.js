...
// data = a function returning the content (actually HTML)
	...
	Vue.component('upgrade', {
		...
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			...
			<span v-else>
				...
				<br><br>{{options.ch?"价格":"Cost"}}: {{ formatWhole(tmp[layer].upgrades[data].cost) }} {{(tmp[layer].upgrades[data].currencyDisplayName ? tmp[layer].upgrades[data].currencyDisplayName : tmp[layer].resource)}}
			</span>
	...
	Vue.component('main-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">{{options.ch?"你拥有":"You have"}} </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{data ? format(player[layer].points, data) : formatWhole(player[layer].points)}}</h2> {{tmp[layer].resource}}<span v-if="layers[layer].effectDescription">, <span v-html="run(layers[layer].effectDescription, layers[layer])"></span></span><br><br></div>
		`
	})
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			<span v-if="tmp[layer].baseAmount"><br>{{options.ch?"你拥有":"You have"}} {{formatWhole(tmp[layer].baseAmount)}} {{tmp[layer].baseResource}}</span>
			<span v-if="tmp[layer].passiveGeneration"><br>{{options.ch?"你每秒获得":"You are gaining"}} {{format(tmp[layer].resetGain.times(tmp[layer].passiveGeneration))}} {{tmp[layer].resource}} {{options.ch?"per second":""}}</span>
			<br><br>
			<span v-if="tmp[layer].showBest && !options.ch">Your best {{tmp[layer].resource}} is {{formatWhole(player[layer].best)}}<br></span>
			<span v-else-if="tmp[layer].showBest && options.ch">你最高拥有过 {{formatWhole(player[layer].best)}} {{tmp[layer].resource}}<br></span>
			<span v-if="tmp[layer].showTotal">{{options.ch?"你总共获得过":"You have made a total of"}} {{formatWhole(player[layer].total)}} {{tmp[layer].resource}}<br></span>
		</div>
		`
	})
			...
			start() {
				if (!this.interval && layers[this.layer].clickables[this.data].onHold) {
					this.interval = setInterval((function() {
						...
						if(this.time >= 0 && run(c.canClick, c)) {//由于奇怪的原因直接改成0好了
							run(c.onHold, c)
						}	
						this.time = this.time+1
					}).bind(this), 50)}
	...
	Vue.component('gridable', {//我入，原版TMT里的grid，背景依据可点击变化是超控的，我直接删掉了。这种玩意还是老老实实自己写比较好——cyxw
		...
		v-bind:class="{ tile: true, can: canClick, locked: !canClick, tooltipBox: canClick,}"
