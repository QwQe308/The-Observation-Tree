...
function save(force) {
	...
	localStorage.setItem(modInfo.id, LZString.compressToBase64(unescape(encodeURIComponent(JSON.stringify(player)))));
	localStorage.setItem(modInfo.id+"_options", LZString.compressToBase64(unescape(encodeURIComponent(JSON.stringify(options)))));
...
function load() {
	...
	if (get === null || get === undefined) {
		...
		player = Object.assign(getStartPlayer(), JSON.parse(decodeURIComponent(escape(LZString.decompressFromBase64(get)))));
		fixSave();
		loadOptions();
	}
...
function loadOptions() {
	...
	if (get2) 
		options = Object.assign(getStartOptions(), JSON.parse(decodeURIComponent(escape(LZString.decompressFromBase64(get2)))));
	else 
		options = getStartOptions()
...
function exportSave() {
	//if (NaNalert) return
	let str = LZString.compressToBase64(unescape(encodeURIComponent(JSON.stringify(player))));
