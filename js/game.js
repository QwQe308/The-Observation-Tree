...
var smallLayersDOM = []
var smallLayersContainerDOM = []
var smallLayersRelatedLayers = []
...
var interval = setInterval(function () {
    ...
    if (ticking) {
        let layersDOM = document.getElementsByClassName("small")
        for (i in layersDOM) {
            if (!layersDOM[i].parentElement) continue
            let container = layersDOM[i].parentElement.parentElement.parentElement
            //save DOM
            smallLayersDOM.push(layersDOM[i])
            smallLayersContainerDOM.push(container)
            let relatedLayers = smallLayersDOM[i].className.split(" ")
            smallLayersRelatedLayers.push(relatedLayers.length>5?relatedLayers[2]:relatedLayers[1])

            //container.style.maxHeight = layers[layersDOM[i].className].includingButtonCount * 50 - 16 + "px"
        ...
        for (i in smallLayersDOM) {
            smallLayersContainerDOM[i].style.maxHeight = document.getElementsByClassName(smallLayersRelatedLayers[i]).length * 50 - 16 + "px"
        }
        /* let others = document.getElementsByClassName("others")
        let container = others[0].parentElement.parentElement.parentElement
        container.style.maxHeight = 3 * 50 - 16 + "px" */
        ticking = false
    }
    if(needCanvasUpdate){
        for (i in smallLayersDOM) {
            let container = smallLayersContainerDOM[i]
            if(container.className.indexOf("extend")+1 && container.className.indexOf("shown")+1){
                container.style.maxHeight = document.getElementsByClassName(smallLayersRelatedLayers[i]).length * 50 - 16 + "px"
            }
        }
